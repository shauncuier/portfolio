...
const Footer = ({ isDarkMode }) => {
  ...
  return (
    <footer className={`py-12 transition-colors duration-300 ${
      isDarkMode ? 'bg-dev-bg text-dev-text' : 'bg-gray-900 text-white'
    }`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
              <p className={`transition-colors ${
                ...
                <a
                  ...
                  className={`text-dev-string hover:underline transition-colors ${
                    isDarkMode ? 'hover:text-primary-400' : 'hover:text-primary-300'
                  }`}
                >
                  "jashedul.shaun@gmail.com"
                </a>
              </p>
              <p className={`transition-colors ${
                isDarkMode ? 'text-dev-textSecondary' : 'text-gray-300'
                ...
                <span className="text-dev-operator">=</span>{' '}
                <span className="text-dev-string">"Dhaka, Bangladesh"</span>
              </p>
              <p className={`transition-colors ${
                isDarkMode ? 'text-dev-textSecondary' : 'text-gray-300'
              }`}>
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          className={`border-t pt-8 flex flex-col md:flex-row justify-between items-center transition-colors ${
            isDarkMode ? 'border-dev-border' : 'border-gray-800'
          }`}
        >
          <p className={`text-sm mb-4 md:mb-0 font-mono transition-colors ${
            isDarkMode ? 'text-dev-textMuted' : 'text-gray-300'
          }`}>
            <span className="text-dev-comment">// </span>
            © {currentYear} Jashedul Islam Shaun. All rights reserved.
          </p>
          <p className={`text-sm flex items-center font-mono transition-colors ${
            isDarkMode ? 'text-dev-textMuted' : 'text-gray-300'
          }`}>
            <span className="text-dev-comment">// </span>
